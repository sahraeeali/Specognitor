...
Executor::Executor(LLVMContext &ctx, const InterpreterOptions &opts,
    ...
    ivcEnabled(false), debugLogBuffer(debugBufferString), ispecEnabled (false) {
        ...
        if(!InputPattern.empty()){
            std::ifstream json_file(InputPattern, std::ifstream::binary);

	        json_file >> klee::jsonVal::jsonFile;
        }
...
ExecutionState* Executor::forkOnCall(ExecutionState &current){
    ExecutionState *spTrueState = &current;
    assert(ispecEnabled);
    ++stats::spStates;
    if(isBTBEnabled) {
    ...
    if (current.isSpeculative) {
        ...
        ExecutionState *parentState = current.pPerentState;

        spTrueState->pPerentState = parentState;
        
        parentState->addSpeState(spTrueState);
...
//     KInstIterator btbPC = state._BTBState->read(ki->info->line, kf, );
//     unsigned btbDst = 0;
//     if(btbPC){
...
void Executor::executeCall(ExecutionState &state, 
    ...
    if (f && f->isDeclaration()) {
        ...
        if(ispecEnabled && isBTBEnabled){
            if (state.isSpeculative && specOn)
            {
                KInstIterator btbPC = state._BTBState->read(ki->info->line, kf, "CALL");
                unsigned btbDst = 0;
                if(btbPC){
                    btbDst = btbPC->info->assemblyLine;
                }
                ...
                //if btb is not empty and they don't match
                    ...
                    state.pc = btbPC;
                    KFunction *btbKF = state._BTBState->getKFunction(ki->info->line);
                    state.pushFrame(state.prevPC, btbKF);
                    //++stats::spStatesPL;
                } else {
                    state.pPerentState->pSpecState = 0;
                    //klee_message("Is spec and BTB is empty %d and father %d", state.tag, state.pPerentState->tag);
                    terminateInfantSpecState(state);
                    --stats::spStatesPL;
                    return;
                }
            }else if(state.isSpeculative){
                state.pushFrame(state.prevPC, kf);
                state.pc = realPC;
            }else{
                state.pushFrame(state.prevPC, kf);
                state._BTBState->update(ki->info->line, realPC, state, kf, "CALL");
                state.pc = realPC;
            }
        }else{
            state.pushFrame(state.prevPC, kf);
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
    ...
    //klee_message("\n============================= tag: %ld,  inst: %d, c-inst: %d", state.tag, ki->info->assemblyLine, ki->info->line);
                                    ...
                                    if(isPHTEnabled){
                                                ...
                                                if (branches.first->pSpecState) {
                                                    ...
                                                    if(!dst){
                                                        ...
                                                        if(!branches.first->isSpeculative){
                                                            ...
                                                            branches.first->pSpecState->monitor->setReturnState(branches.first->pPerentState);
                                                        }
                                                        ++stats::spStatesCommon;
                                                    }
                                                }
                                                ...
                                                if (branches.second->pSpecState) {
                                                    ...
                                                    if(!dst){
                                                        ...
                                                        if(!branches.second->isSpeculative){
                                                            ...
                                                            branches.second->pSpecState->monitor->setReturnState(branches.second->pPerentState);
                                                        }
                                                        ++stats::spStatesCommon;
                                                    }else{
                                        ...
                                        if(ispecEnabled){
                                            ...
                                            if(branches.first){
                                                ...
                                                KInstIterator btbPC = branches.first->_BTBState->read(ki->info->line, kf, "BR");
                                                unsigned btbDst = 0;
                                                if(btbPC){
                                                    btbDst = btbPC->info->assemblyLine;    
                                                }
                                                ...
                                                if (branches.first->pSpecState){
                                                    ...
                                                    if((btbDst == realDst) || (btbDst == 0)){
                                                        ...
                                                        if( !branches.first->isSpeculative ){
                                                            unsigned entry = kf->basicBlockEntry[bi->getSuccessor(0)];
                                                            KInstIterator pc = &kf->instructions[entry];
                                                            branches.first->_BTBState->update(ki->info->line, pc, *branches.first, kf, "BR");
                                                        }
                                            ...
                                            if(branches.second){
                                                ...
                                                //reading btb to check the destination. btbDst is the assembly line of the destination.
                                                KInstIterator btbPC = branches.second->_BTBState->read(ki->info->line, kf, "BR");
                                                unsigned btbDst = 0;
                                                if(btbPC){
                                                    btbDst = btbPC->info->assemblyLine;    
                                                }

                                                //extracting assembly line of bi->getSuccessor(1)
                                                ...
                                                if (branches.second->pSpecState) {
                                                    ...
                                                    if(btbDst == realDst){
                                                        ...
                                                        if(!branches.second->isSpeculative){
                                                            unsigned entry = kf->basicBlockEntry[bi->getSuccessor(1)];
                                                            KInstIterator pc = &kf->instructions[entry];
                                                            branches.second->_BTBState->update(ki->info->line, pc, *branches.second, kf, "BR");
                                                        }
        ...
        case Instruction::Call: {
                                    ...
                                    if (f) {
                                        ...
                                        if(state.isSpeculative)
                                            ++stats::spStatesPL;
                                        */
                                        ExecutionState* newSp;
                                        uint64_t ttt;
                                        if(ispecEnabled && isBTBEnabled){
                                            newSp = forkOnCall(state);
                                            ttt = newSp->tag;
                                            executeCall(*newSp, ki, f, arguments, true);
                                        }
                                        executeCall(state, ki, f, arguments, false);
                                        state.monitor->update(uinfo);
                                        if(ispecEnabled && isBTBEnabled && newSp && (ttt == newSp->tag)){
                                            //klee_message("========================== executing direct call on %ld from %d", newSp->tag, state.isSpeculative);
                                            newSp->monitor->update(uinfo);
                                            if(!state.isSpeculative){
                                                newSp->monitor->setReturnState(&state);
                                            }else{
                                                newSp->monitor->setReturnState(state.pPerentState);
                                            }
                                        }
                                        ...
                                        do {
                                            ...
                                            if (res.first) {
                                                ...
                                                if (legalFunctions.count(addr)) {
                                                    ...
                                                    if(ispecEnabled && res.first->pSpecState && isBTBEnabled){
