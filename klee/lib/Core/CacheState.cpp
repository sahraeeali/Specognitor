...
/// if it's a cold miss. Returns true iff it's a cold miss. returns false iff it's a reload miss or might be a hit.
...
// returns true iff it's reload miss. returns false if it's a unique miss or it's 
bool CacheState::reloadConstr(ref<Expr> &addr1, ref<Expr> &addr2, ref<Expr> &result) {
    if (isa<ConstantExpr>(addr1) && isa<ConstantExpr>(addr2)) {
        ...
        // if it is not a reload, return constraint with bool value True;
        result = ConstantExpr::alloc(1, Expr::Bool);
        return false;
    }
    ref<Expr> setConstr = CacheUtil::generateSetCnstr(addr1, addr2);
...
void CacheState::store(ref<Expr> &addr, const InstructionInfo *info) {
    ...
    ///Store operations are not executed until commit stage.
...
void CacheState::handleAccess(ref<Expr> &addr, const InstructionInfo *info) {
    ...
    for (auto &mt : targets) {
        ...
        /// if addr is already in conf_addrs of target, go to next target. O.w. go on.
...
// verify whether there are sensitive data leakage. returns true in case of leakage. false in case of no sensitive value in the cache.
uint8_t CacheState::verifyCacheSideChannel(TimingSolver *solver) {
