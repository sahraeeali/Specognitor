...
StackFrame::~StackFrame() {
  //klee_message("delete a stack for instruction %ld", caller->info->assemblyLine);
  delete[] locals; 
}
...

{
  ...
  _BTBState = new BTBState(*this);
  _PHTState = new PHTState(*this);
...
ExecutionState::ExecutionState(const ExecutionState& state):
    ...
    //ctrlSpecMemOp(state.ctrlSpecMemOp),
...

{
  ...
  _BTBState = new BTBState(*this, *(state._BTBState));
  _PHTState = new PHTState(*this, *(state._PHTState));
  monitor = std::make_shared<Monitor>(this ,*(state.monitor));
...

{
  ...
  _PHTState = new PHTState(*this, *(state._PHTState));
  monitor = std::make_shared<Monitor>(this, *(state.monitor));
  //klee_message(" --------------------- Spec Copy: monitor pattern 0 ttlPrev%d: %d \t ttlNew%d: %d ---------------", state.tag, tag, monitor->(*patterns[0].begin())->);
  for (unsigned int i=0; i<symbolics.size(); i++)
    symbolics[i].first->refCount++;
...
ExecutionState *ExecutionState::specBranch(int sew) {
  ...
  /*
  klee_message("\n============================= Create spec %ld from %ld", speState->tag, this->tag);
  if(isSpeculative){
    klee_message("\n============================= parent is: %ld", pPerentState->tag);
  }
  */
  return speState;
}
...
ExecutionState *ExecutionState::branch() {
  ...
  /*
  klee_message("\n============================= Create real %ld from %ld", falseState->tag, this->tag);
  if(isSpeculative){
    klee_message("\n============================= parent is: %ld", pPerentState->tag);
  }
  */
...
{
    //klee_message("Add a speculative state: -> %d", _state->tag);
	//assert(false);
    specStates.push_back(_state);
}
