...
namespace {
  //editing. The input pattern file.
  //cl::opt<std::string>
  //InputPattern(cl::desc("<input pattern file>"), cl::Positional, cl::init("-"));
...
int main(int argc, char **argv, char **envp) {
  ...
  uint64_t spStatesPL = 
      *theStatisticManager->getStatisticByName("SpstatesPL");
  uint64_t spStatesCommon = 
      *theStatisticManager->getStatisticByName("SpstatesCommon");
  ...
  stats << "KLEE: done: sp states without Prediction Logic = " 
        <<  spStates << "\n";
  
  stats << "KLEE: done: sp states with Prediction Logic = " 
        <<  spStatesPL << "\n";
  if(spStates != 0){
      stats << "KLEE: done: sp states reduction rate = " 
        <<  (int)((1.0-((float)spStatesPL/spStates))*100) << "% \n";

  }
  stats << "KLEE: done: Common sp states = " << spStatesCommon << "\n";
